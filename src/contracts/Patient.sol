...
contract Patient {
    ...
    function saveDoctor(string memory _docName, address _doctorAcc, string memory _docSpeciality, string memory _docHospital) public {
        patientData storage p = patients[msg.sender];
        doctors[msg.sender][p.doctorCount] = doctorData(_doctorAcc, _docName, _docSpeciality, _docHospital);
        p.doctorCount ++;
    }
